...
function ProcessCoke()
	...
	TriggerServerEvent('esx_drugs1:processCoke')
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.CokeProcessing.coords, false) > 4 then
			...
			TriggerServerEvent('esx_drugs1:cancelProcessing')
			break
		end
	end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_drugs1:canPickUp', function(canPickUp)
					if canPickUp then
						TaskStartScenarioInPlace(playerPed, 'world_human_gardener_plant', 0, false)
						...
						TriggerServerEvent('esx_drugs1:pickedUpCoke')
					else
...
function SpawnCokePlants()
	while spawnedCokes < 25 do
		...
		ESX.Game.SpawnLocalObject('prop_plant_cane_02b', cokeCoords, function(obj)
