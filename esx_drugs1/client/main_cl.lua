...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.CokeDealer.coords, true) < 1.5 then
...
function OpenDrug1Shop()
	ESX.UI.Menu.CloseAll()
	local elements = {}
	menuOpen = true

	for k, v in pairs(ESX.GetPlayerData().inventory) do
		local price = Config.CokeDealerItems[v.name]
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'drug1_shop', {
		title    = ("Revente de drogues durs"),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		TriggerServerEvent('esx_drugs1:sellCoke', data.current.name, data.current.value)
...
function OpenBuyLicenseMenu(licenseName)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_shop', {
		...
		if data.current.value ~= 'no' then
			ESX.TriggerServerCallback('esx_drug1:buyLicense', function(boughtLicense)
