...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.AcetoneDealer.coords, true) < 2.0 then
...
function OpenDrug2Shop()
	ESX.UI.Menu.CloseAll()
	local elements = {}
	menuOpen = true

	for k, v in pairs(ESX.GetPlayerData().inventory) do
		local price = Config.AcetoneDealerItems[v.name]
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'drug2_shop', {
		title    = _U('dealer_title'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		TriggerServerEvent('esx_drugs2:sellAcetone', data.current.name, data.current.value)
...
function OpenBuyLicenseMenu(licenseName)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_shop', {
		...
		if data.current.value ~= 'no' then
			ESX.TriggerServerCallback('esx_drug2:buyLicense', function(boughtLicense)
