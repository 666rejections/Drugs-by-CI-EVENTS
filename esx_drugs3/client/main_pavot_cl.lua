...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.PavotDealer.coords, true) < 1.5 then
...
function OpenDrug3Shop()
	ESX.UI.Menu.CloseAll()
	local elements = {}
	menuOpen = true

	for k, v in pairs(ESX.GetPlayerData().inventory) do
		local price = Config.PavotDealerItems[v.name]
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'drug3_shop', {
		title    = _U('dealer_title'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		TriggerServerEvent('esx_drugs3:sellPavot', data.current.name, data.current.value)
...
function OpenBuyLicenseMenu(licenseName)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_shop', {
		...
		if data.current.value ~= 'no' then
			ESX.TriggerServerCallback('esx_drug3:buyLicense', function(boughtLicense)
