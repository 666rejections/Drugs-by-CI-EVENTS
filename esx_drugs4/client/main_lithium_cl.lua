...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.LithiumDealer.coords, true) < 5.0 then
...
function OpenDrug4Shop()
	ESX.UI.Menu.CloseAll()
	local elements = {}
	menuOpen = true

	for k, v in pairs(ESX.GetPlayerData().inventory) do
		local price = Config.LithiumDealerItems[v.name]
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'drug1_shop', {
		...
		TriggerServerEvent('esx_drugs4:sellLithium', data.current.name, data.current.value)
